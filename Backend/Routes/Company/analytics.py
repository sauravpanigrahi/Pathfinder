...
def create_bar_chart_base64(labels: list, values: list, title: str, xlabel: str, ylabel: str, color: str = '#36A2EB') -> str:
    ...
    for i, v in enumerate(values):
        plt.text(i, v + max(values) * 0.01 if max(values) > 0 else 0.1, str(v), ha='center', va='bottom', fontweight='bold')
...
@router.get("/{company_uid}")
async def get_company_analytics(company_uid: str, db: Session = Depends(get_db)):
    """
    Get comprehensive analytics for a company including:
    ...
    try:
        ...
        company = db.query(CompanyDetails).filter(CompanyDetails.uid == company_uid).first()
        if not company:
            raise HTTPException(status_code=404, detail="Company not found")
        
        # Get all applications for this company
        applications = db.query(Applications).filter(Applications.comp_uid == company_uid).all()
        ...
        # Count interviews scheduled for this company
        interviews_scheduled = db.query(Interview).filter(
            and_(
                Interview.company_uid == company_uid,
        ...
        # Get job opening field ratio (by job type) for this company
        company_jobs = db.query(Jobs).filter(
            and_(
                Jobs.uid == company_uid,
                Jobs.status == 'Active'
            )
        ).all()
        ...
        # Get total jobs posted (including inactive)
        total_jobs_posted = db.query(Jobs).filter(Jobs.uid == company_uid).count()
