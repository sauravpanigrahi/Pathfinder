...
const Jobs = () => {
  ...
  const fetchJobsWithMatch = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:8000/jobs/with-match/${userID}`,
 ...
 const openJobs = useMemo(
  ...
  return openJobs
    ...
    .filter((job) => (activeTypes.length ? activeTypes.includes(job.type) : true))
    .filter((job) => (activeLocations.length ? activeLocations.includes(job.location) : true));
...
const handleDeleteResume = async () => {
  ...
  try {
    setDeleting(true);
    await axios.delete(
      `http://localhost:8000/student/resume/delete/${userID}`,
      { withCredentials: true }
    );
      ...
      <div className="mx-auto max-w-8xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">Explore the latest openings curated for you</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button className=' p-2 rounded rounded-xl bg-blue-600 hover:bg-blue-700 text-white' onClick={()=>navigate("/interviewprep")}>Interview Prep</button>
          </div>
        </div>
            ...
            {filteredJobs.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-2">
                {sortedPaginatedJobs.map((job) => (
                  <JobCard key={job.id} job={job} Resume={resume}  isTopMatch={job.id === topMatchId} />
                ))}
              </div>
            )}
          </div>
